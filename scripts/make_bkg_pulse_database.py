...
def load_traces(root_files,
    ...
    `tags`
    type        : list[int]
    description : Tags corresponding to the polarization where a candidate pulse was triggered: 1 = X, 2 = Y, 3 = XY.
    ...
    for i, root_file, entry, month, tag in zip(range(n_files),root_files,entries,months,tags):
        ...
        trace_filtered = filter_trace(trace)
    ...
    return traces[mask], len_original_trace[mask], trigger_flags[mask]
...
if __name__ == '__main__':
    ...
    traces, len_original_trace, trigger_flags = load_traces(file_contents['root_files'],
