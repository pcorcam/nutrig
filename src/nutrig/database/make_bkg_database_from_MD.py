...
The following steps are followed:
    1. Take non-filtered GP80 MD data from run 145 (4 February 2025).
...
RUN_NUMBER          = 145
DATA_DIR            = '/sps/grand/data/gp80/GrandRoot/2025/02' #02
DATA_FILES          = sorted( glob.glob( os.path.join( DATA_DIR,f'GP80_*_RUN{RUN_NUMBER}_MD_*.root' ) ) )
FLOAT_CH            = 0
N_CHANNELS          = 4
N_SAMPLES           = 1024
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Apply a Gaussian fade-in and fade-out window to the traces to remove ugly filter artifacts
        # NOTE: do this AFTER the filtering. Otherwise you will get a bunch of artificial pulses in the beginning of the trace
        logger.info(f'>>> Applying Gaussian fade-in/fade-out to first/last {SAMPLES_FROM_EDGE} samples of the traces...')
        window = tools.get_gauss_window_edges(SAMPLES_FROM_EDGE,N_SAMPLES)
        #traces_filtered *= window
        ...
        # Uses the effective time per trace over which we scan for FLT-0 triggers
        logger.info(f'>>> Computing FLT-0 trigger rates...')
        t_eff = 1e-9 * ( N_SAMPLES*2 - DICT_TRIGGER_PARAMS['t_period'] - SAMPLES_FROM_EDGE*2 ) # [s]
        res_FLT0_trigger_rate = tools.compute_FLT0_trigger_rate_MD(res_FLT0['n_FLT0'],
                                                                   res_FLT0['FLT0_flags'],
                                                                   data['du_ids'],
                                                                   data['du_seconds'],
                                                                   data['du_nanoseconds'],
        ...
        np.savez(filtered_file,
                 traces=traces_filtered[mask_FLT0_pass]*window,
