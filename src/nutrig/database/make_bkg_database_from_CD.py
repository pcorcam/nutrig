...
def load_data(data_file,
    ...
    with rt.DataFile(data_file) as df:
        ...
        for entry in range(n_entries):
            ...
            du_ids_event = tadc.du_id
            
            # Select a random DU ID from the event and make sure it's not one to exclude
            do_while = True
            while do_while:
                idx = np.random.randint( len(du_ids_event) )
                if du_ids_event not in du_ids_exclude:
                    do_while = False

            # Only one DU per entry
            traces[entry]         = tadc.trace_ch[idx]
            du_ids[entry]         = tadc.du_id[idx]
            du_seconds[entry]     = tadc.du_seconds[idx]
            du_nanoseconds[entry] = tadc.du_nanoseconds[idx]
