...
def get_snr_with_file_tags(sig_files,
    ...
    return snr[:n_entries_tot], file_idx[:n_entries_tot], file_entry[:n_entries_tot]


def select_trace_idcs(snr,
                      n_select,
                      bin_edges=np.arange(3.,8.25,.25),
    ...
    logger.info(f'Weighting the bins with the "empirical" function y = 10^({power_law_index}x)')
...
if __name__ == '__main__':
    ...
    snr, file_idx, file_entry = get_snr_with_file_tags(input_files)

    #-#-#- Select random trace indices based on the SNR  -#-#-#
    idcs_sel = select_trace_idcs(snr,n_traces_train+n_traces_test,rng=rng)
    ...
    files           = np.zeros( (len(idcs_sel), ),dtype='<U100')
