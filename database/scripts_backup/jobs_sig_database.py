...
output_dir = f'/sps/grand/pcorrea/nutrig/database/sig/{sim_soft}_{primary.lower()}_{rf_chain}'
...
if __name__ == '__main__':
    ...
    for i in range(n_jobs):
        ...
        for j in range(n_files_per_job):
            try:
                script_text_new = '\n\npython3 /pbs/home/p/pcorrea/grand/nutrig/database/scripts/make_sig_database.py {} -o {} -s {}'
                script_text    += script_text_new.format(input_files[file_idx],output_dir,file_idx)
                file_idx       += 1
            except:
                continue
