...
threshold1              = 55 # [ADC counts]
threshold2              = 35 # [ADC counts]
...
def manage_args():
    ...
    parser.add_argument('-n',
                        '--noise_dir',
                        type=str,
                        default='/sps/grand/pcorrea/nutrig/database/bkg/gp13_pretrigger_stationary/',
...
if __name__ == '__main__':
    ...
    sim_traces, du_xyz, du_seconds, du_nanoseconds = tools.get_sim_traces(input_file)
    # sim_traces_filtered = tools.filter_traces(sim_traces,
    #                                           freq_highpass=freq_highpass,
    #                                           freqs_notch=freqs_notch,
    #                                           bw_notch=bw_notch)

    #-#-#- Collect random noise traces from measured data -#-#-#
    noise_traces, noise_file, noise_file_entries = tools.get_noise_traces(noise_dir,sim_traces.shape[0],rng=rng)
    # noise_traces_filtered = tools.filter_traces(noise_traces,
    #                                             freq_highpass=freq_highpass,
    #                                             freqs_notch=freqs_notch,
    #                                             bw_notch=bw_notch)
    ...
    sig_traces, inj_pulse_times = tools.add_sim_to_noise(sim_traces,
                                                         noise_traces,
