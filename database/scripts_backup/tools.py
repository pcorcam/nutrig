...
def digitize(voltage_traces,
    ...
    adc_traces = np.trunc(voltage_traces/adc_to_voltage)
...
def get_sim_traces(voltage_file,
    ...
    du_xyz         = np.array(trun.du_xyz) + np.array(tshower.shower_core_pos) # GP300 coords
    du_seconds     = np.array(tvoltage.du_seconds)
    du_nanoseconds = np.array(tvoltage.du_nanoseconds)
    ...
    return traces, du_xyz, du_seconds, du_nanoseconds
...
def add_sim_to_noise(sim_traces,
    ...
    max_sample_sim  = np.argmax(np.abs(sim_traces),axis=2)[:,0] # position same for each polarization
    pulse_start_inj = max_sample_inj - max_sample_sim + n_samples
...
def get_snr(sig_traces,
    ...
    for i, sig_trace in enumerate(sig_traces):
        ...
        max_pulse      = np.abs( sig_trace[...,inj_pulse_time] )
