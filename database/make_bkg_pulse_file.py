...
if __name__ == '__main__':
    ...
    pretrig_flags = np.zeros( (0,),dtype=int )
    ...
    for i, input_file in enumerate(input_files):
        ...
        n_traces = len(f['traces'])

        mask         = np.arange(n_traces,dtype=int)
        max_per_file = 15
        if n_traces > max_per_file:
            logger.info(f'Picking {max_per_file} random traces from this file')
            mask = np.random.choice(mask,size=max_per_file,replace=False)

        traces        = np.vstack( (traces,f['traces'][mask]) )
        pretrig_flags = np.hstack( (pretrig_flags,f['pretrig_flags'][mask]) )
        pretrig_times = np.hstack( (pretrig_times,f['pretrig_times'][mask]) )
        entries       = np.hstack( (entries,f['entries'][mask]) )

        file_arr      = np.array( [ input_file for i in range( len(f['entries'][mask] ) ) ],dtype='<U100' )
    ...
    n_traces_max = 20000
    idx_split    = int(n_traces_max/2)

    np.random.shuffle(mask_rand)
    mask_rand = mask_rand[:n_traces_max]
